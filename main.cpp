...
const int maxD = 2100000;
const int dataRanges[6] = {10, 100, 1, INT_MAX, INT_MIN + 100, INT_MAX - 100};
const int dataRangesCount = 3;
const int minElements = 100000;
const int testPrecision = 100;
...
void displayMenuArray(Array<int> *array)
{
    ...
    do
    {
        ...
        case 7: //funckja służąca do testowania.
        {
            ...
            {
                testOutput << "wielkosc struktury\t[10,100]\t[1,INT_MAX]\t[INT_MIN+100,INT_MAX-100]\n";
                for (int i = minElements; i <= maxD; i += 250000)
                {
                    testOutput << i;
            ...
            testOutput << "Wyszukiwanie - klucz poza przedziałem:\n";
            {
                ...
                for (int i = minElements; i <= maxD; i += 250000)
                {
                    ...
                    for (int k = 0; k < dataRangesCount; k++)
                    {
                        ...
                        for (int j = 0; j < testPrecision; j++)
                        {
                            array->isElementInArray(-INT_MAX + 20);
                        }
                        auto endTime = chrono::high_resolution_clock::now();
                        auto sumaCzasow = (chrono::duration_cast<chrono::nanoseconds>(endTime - startTime).count()) / testPrecision;
                        testOutput << "\t" << sumaCzasow;
            ...
            testOutput << "Wyszukiwanie - klucz poza przedziałem:\n";
            {
                ...
                for (int i = minElements; i <= maxD; i += 250000)
                {
                    ...
                    for (int k = 0; k < dataRangesCount; k++)
                    {
                        ...
                        for (int j = 0; j < testPrecision; j++)
                        {
                            heap->findElementPosition(-INT_MAX + 20);
                        }
                        auto endTime = chrono::high_resolution_clock::now();
                        auto timeResult = (chrono::duration_cast<chrono::nanoseconds>(endTime - startTime).count()) / testPrecision;
                        testOutput << "\t" << timeResult;
        ...
        case 8: // test function
        {
            ...
            testOutput << "Usuwanie:\n";
            {
                ...
                for (int i = 1000; i <= 21000; i += 2500)
                {
                    ...
                    {
                        uniform_int_distribution<> dis(dataRanges[k * 2], dataRanges[k * 2 + 1]);
                        tree->generateTree(i - 100, dataRanges[k * 2], dataRanges[k * 2 + 1]);
                        int values[100];
                        for (int i = 0; i < 100; i++)
                        {
                            values[i] = dis(gen);
